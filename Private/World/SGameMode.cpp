...
void ASGameMode::AddArrayDuplicator()
{
    if(GEngine){
        GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Green,TEXT("Added a new array modifier"));
    }
    
   
    AArrayModifier* new_array_modifier;
    if(GetWorld()){
        FVector new_location = FVector(0,0,0);        
        FActorSpawnParameters SpawnInfo;
        AArrayModifier *instance = GetWorld()->SpawnActor<AArrayModifier>( new_location,FRotator::ZeroRotator,SpawnInfo);
        instance->count = 4;
         UStaticMesh* floor = LoadObject<UStaticMesh>(nullptr,TEXT("/Game/Pavon/asuncion/costanera/piso"));
         instance->mesh_to_duplicate = floor;
    }
}
void ASGameMode::LogGameStateTest()
{
    if(GEngine){
        GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Green,TEXT("Bluprint callable"));
    }
}
