...
void ASCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
    ...
    PlayerInputComponent->BindAction("EnterInCar",IE_Pressed,this,&ASCharacter::OnEnterInCarPressed);
}
...
void ASCharacter::Use()
{
	...
	if (Role == ROLE_Authority)
	{
		...
		if (Usable)
		{
            GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Green,TEXT("Use"));
			Usable->OnUsed(this);
		}else
            GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Red,TEXT("UNo usable"));
...
void ASCharacter::OnEnterInCarPressed()
{
    if (bEnterInCar == true){
        bEnterInCar = false;
    }
